 ...
 */
#define NAMESPACE_FOR_HASH_FUNCTIONS farmhashna
...
int main(int argc, char *argv[]) {
	...
	for (uint64_t size = 0; size <= maxSize; ++size) {
		...
		for (uint64_t i = 0; i < numExamplesPerSize; ++i) {
			...
			uint64_t hash0 = farmhashna::Hash64((char*) (&data[0]), size);
			uint64_t hash1 = farmhashna::Hash64WithSeed((char*) (&data[0]),
					size, seed1);
			uint64_t hash2 = farmhashna::Hash64WithSeeds((char*) (&data[0]),
					size, seed1, seed2);
