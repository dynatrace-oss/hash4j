 ...
 * </ul>
 */
public final class UltraLogLog extends DistinctCounter<UltraLogLog, UltraLogLog.Estimator> {
  ...
  public double getDistinctCountEstimate() {
    return getDistinctCountEstimate(Estimator.SMALL_RANGE_CORRECTED_4_GRA_ESTIMATOR);
  }
  ...
  // visible for testing
  enum Estimator implements DistinctCounter.Estimator<UltraLogLog> {
      ...
      public double estimate(UltraLogLog ultraLogLog) {
        ...
        if (beta > 0) {
          ...
          if (alpha > 0) {
            ...
            if (c[0] > 0)
              sum +=
                  c[0] * (z + KAPPA_1 * (z2 + KAPPA_1 * (KAPPA_2 + xi(KAPPA_1, z2 * z2 * z) / z)));
    ...
    MAXIMUM_LIKELIHOOD_ESTIMATOR {
      ...
      private double contribute(int r, int[] b, int p) {
        int r2 = r - (p << 2) - 4;
        if (r2 < 0) {
          if (r2 == -8) {
            b[1] += 1;
