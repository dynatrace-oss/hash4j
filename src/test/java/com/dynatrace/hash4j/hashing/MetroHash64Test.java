...
class MetroHash64Test extends AbstractHasher64Test {

  @Override
  protected List<? extends Hasher64> createHashers() {
    return List.of(Hashing.metroHash64(), Hashing.metroHash64(0xd6be88efec43eda7L));
  }
  ...
  protected void calculateHashForChecksum(byte[] seedBytes, byte[] hashBytes, byte[] dataBytes) {
    ...
    long hash0 = Hashing.metroHash64().hashBytesToLong(dataBytes);
    long hash1 = Hashing.metroHash64(seed).hashBytesToLong(dataBytes);
  ...
  protected void calculateHashForChecksum(
    ...
    long hash0 = Hashing.metroHash64().hashBytesToLong(o, off, len, byteAccess);
    long hash1 = Hashing.metroHash64(seed).hashBytesToLong(o, off, len, byteAccess);
  ...
  protected void calculateHashForChecksum(byte[] seedBytes, byte[] hashBytes, CharSequence c) {
    ...
    long hash0 = Hashing.metroHash64().hashCharsToLong(c);
    long hash1 = Hashing.metroHash64(seed).hashCharsToLong(c);
  ...
  protected List<HashStream> getHashStreams(byte[] seedBytes) {
    long seed = getLong(seedBytes, 0);
    return List.of(Hashing.metroHash64().hashStream(), Hashing.metroHash64(seed).hashStream());
  }
