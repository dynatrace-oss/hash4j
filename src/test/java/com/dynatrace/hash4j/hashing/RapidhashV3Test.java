...
class RapidhashV3Test extends AbstractHasher64Test {

  @Override
  protected List<? extends Hasher64> createHashers() {
    return List.of(Hashing.rapidhashV3(), Hashing.rapidhashV3(0x52a4bc2b51cd5d00L));
  }
  ...
  protected void calculateHashForChecksum(byte[] seedBytes, byte[] hashBytes, byte[] dataBytes) {
    ...
    long hash0 = Hashing.rapidhashV3().hashBytesToLong(dataBytes);
    long hash1 = Hashing.rapidhashV3(seed).hashBytesToLong(dataBytes);
  ...
  protected void calculateHashForChecksum(
    ...
    long hash0 = Hashing.rapidhashV3().hashBytesToLong(o, off, len, byteAccess);
    long hash1 = Hashing.rapidhashV3(seed).hashBytesToLong(o, off, len, byteAccess);
  ...
  protected void calculateHashForChecksum(byte[] seedBytes, byte[] hashBytes, CharSequence c) {
    ...
    long hash0 = Hashing.rapidhashV3().hashCharsToLong(c);
    long hash1 = Hashing.rapidhashV3(seed).hashCharsToLong(c);
  ...
  protected List<HashStream> getHashStreams(byte[] seedBytes) {
    long seed = getLong(seedBytes, 0);
    return List.of(Hashing.rapidhashV3().hashStream(), Hashing.rapidhashV3(seed).hashStream());
  }
