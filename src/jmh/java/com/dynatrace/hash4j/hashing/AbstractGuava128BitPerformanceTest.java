...
public abstract class AbstractGuava128BitPerformanceTest extends AbstractPerformanceTest {
  ...
  protected void hashCharsIndirect(String s, Blackhole blackhole) {
    blackhole.consume(createHashFunction().hashObject(s, GUAVA_CHARS_FUNNEL));
  }

  @Override
  protected void hashCharsUTF8Indirect(String s, Blackhole blackhole) {
    blackhole.consume(createHashFunction().hashObject(s, GUAVA_CHARS_UTF8_FUNNEL));
  }

  @Override
  protected void hashCharsUTF8IndirectViaGetBytes(String s, Blackhole blackhole) {
    blackhole.consume(createHashFunction().hashObject(s, GUAVA_CHARS_UTF8_VIA_GET_BYTES_FUNNEL));
  }

  @Override
  protected void hashBytesIndirect(byte[] b, Blackhole blackhole) {
    blackhole.consume(createHashFunction().hashObject(b, GUAVA_BYTES_FUNNEL));
  }
